...
Cvine.selection <- function(udata, candidate.number, selection.step, family_set = "tll",
  ...
  ## requires R packages "rvinecopulib" and "doParallel"
  ## ***************************************************************************************************
  ## ***************************************************************************************************
  start.time <- Sys.time()
  p <- dim(udata)[2] - 1
  ...
  if("rvinecopulib" %in% rownames(installed.packages()) == FALSE) install.packages("rvinecopulib")
 ...
 # }
  ...
  ## ***************************************************************************************************
     ...
     loglik.large.new.tmp <- 
       foreach(cand.pointer = cand.index, .packages = c("rvinecopulib", "doParallel"), 
  ...
  #   for(cand.pointer in cand.index) {
           ...
           if(selection.pointer == 1)  
             loglik.component1 <- logLik(bicop(cbind(udata[, (cand.pointer + 1)], udata[, 1]), 
                                               family_set = family_set, nonpar_method = nonpar_method))
           ...
           dist.fit.tmp.1 <- bicop(cbind(loglik.large.old$conditional.dist[, (cand.pointer + 1)], 
                                          loglik.large.old$conditional.dist[, 1]),
                                   family_set = family_set, nonpar_method = nonpar_method)
    ...
    #               loglik.large.old$conditional.dist[, 1]), 
    #               1, function(x) hbicop(x, 1, dist.fit.tmp.1))
      hevaluate <- hbicop(cbind(loglik.large.old$conditional.dist[, (cand.pointer + 1)],
                               loglik.large.old$conditional.dist[, 1]), 1, dist.fit.tmp.1)
      ...
      loglik.compare.tmp <- 
        foreach(k.pointer = k.index.reduced, .packages = "rvinecopulib", 
                .combine = function(...) mapply('cbind', ..., SIMPLIFY=FALSE),
                .multicombine=TRUE, .inorder = TRUE) %dopar% {
                  
                  
                  dist.fit.tmp.2 <- 
                    bicop(cbind(loglik.large.old$conditional.dist[, (cand.pointer + 1)], 
                ...
                #  hevaluate <- apply(cbind(loglik.large.old$conditional.dist[, (cand.pointer + 1)],
                #                           loglik.large.old$conditional.dist[, (k.pointer + 1)]), 
                #                     1, function(x) hbicop(x, 1, dist.fit.tmp.2))
                  ...
                  loglik.component2 <- logLik(bicop(cond.distribution, family_set = family_set,
  ...
  if(selection.pointer == 1)  
    loglik.component1 <- logLik(bicop(cbind(udata[, (cand.pointer + 1)], udata[, 1]), family_set = family_set,
                                      nonpar_method = nonpar_method))
  else loglik.component1 <- recycle.loglik[cand.pointer]
  ...
  loglik.compare.tmp <- 
    foreach(k.pointer = k.index, .packages = "rvinecopulib", 
              .combine = function(...) mapply('cbind', ..., SIMPLIFY=FALSE),
              .multicombine=TRUE, .inorder = TRUE) %dopar% {
...
#selected.vars <- 
#  Cvine.selection(udata = udata, candidate.number = 5, selection.step = 6, method = "spearman",
#                  k.redu.method = "none", k.redu.percent = c(.2, .1))
#selected.vars
